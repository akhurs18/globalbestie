...
/**
 * Customer sign-in for /my-orders. The OMS owns the codes, the sessions and what a
 * customer may see; this site only passes messages and keeps the session token in its
 * own httpOnly cookie, so it never reaches the shopper's JavaScript.
 */
export function requestPortalCode(phone, customerIp) {
  return call('/api/public/portal/request-code', { method: 'POST', body: { phone }, customerIp });
}

export function verifyPortalCode(phone, code, customerIp) {
  return call('/api/public/portal/verify', { method: 'POST', body: { phone, code }, customerIp });
}

export function portalOrders(token, customerIp) {
  return call('/api/public/portal/orders', { headers: { 'X-Portal-Token': token }, customerIp });
}

export function portalSignOut(token, customerIp) {
  return call('/api/public/portal/orders', { method: 'DELETE', headers: { 'X-Portal-Token': token }, customerIp });
}
